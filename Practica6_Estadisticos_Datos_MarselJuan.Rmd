...
library(tidyverse)
#Usar factor
#Cuando hago un factor, guardo un número, por lo que cuando haga operaciones, será con el número no con su nombre
...
datos %>%
  ...
  geom_density() +
  geom_vline(aes(xintercept = mean(valor, na.rm=T)), color = "blue", linetype = "dashed", size = 1)
...
mosaicplot(tabla_contingencia,
           color = TRUE,
           main = "Tabla de contingencia ALG vs ANM",
           xlab = "ALG",
           ylab = "ANM")
...
datos %>%
  select(ends_with('.num')) %>%
  mutate(ALG_sqrt = sqrt(ALG.num)) %>%
  summarise(
    pearson_sqrt = cor(ALG_sqrt, MD.num, method = "pearson"),
    spearman_sqrt = cor(ALG_sqrt, MD.num, method = "spearman")
  )

# Correlación con sqrt(ALG.num)
cor_pearson_sqrt <- cor(sqrt(datos$ALG.num), datos$MD.num, method = "pearson")
cor_spearman_sqrt <- cor(sqrt(datos$ALG.num), datos$MD.num, method = "spearman")
...
# Seleccionar solo las columnas numéricas de notas
notas <- datos %>% select(ends_with(".num"))

# 1. Matriz de covarianza (no acotada)
covarianza <- cov(notas, use = "complete.obs")
print("Matriz de covarianza:")
print(covarianza)

# 2. Matriz de correlación de Pearson (lineal)
cor_pearson <- cor(notas, use = "complete.obs", method = "pearson")
print("Matriz de correlación de Pearson:")
print(cor_pearson)

# 3. Matriz de correlación de Spearman (rangos)
cor_spearman <- cor(notas, use = "complete.obs", method = "spearman")
print("Matriz de correlación de Spearman:")
print(cor_spearman)

# 4. Visualización con pairs() (base R)
pairs(notas, main = "Matriz de dispersión (pairs)")

# 5. Visualización con ggpairs() (GGally)
ggpairs(notas, title = "Matriz de correlación con GGally")
...
tabla_alg_anm <- table(datos$ALG.cat, datos$ANM.cat)

# Test Chi-cuadrado de Pearson
chi_alg_anm <- chisq.test(tabla_alg_anm, correct = FALSE)
chi_alg_anm
...
# Tabla de contingencia
tabla_alg_sex <- table(datos$ALG.cat, datos$Sex)

# Test Chi-cuadrado de Pearson
chi_alg_sex <- chisq.test(tabla_alg_sex, correct = FALSE)
chi_alg_sex
